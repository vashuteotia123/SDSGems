...
class diamondfilter(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.GET.get('shape'):
            all_diaobjects,shape=self.get_by_shape(self.request.GET.get('shape'),all_diaobjects)
        ...
        if self.request.GET.get('cut'):
            all_diaobjects,cut=self.get_by_cut(self.request.GET.get('cut'),all_diaobjects)
            context['total_count']=self.get_object_count(all_diaobjects)
            paginator=Paginator(all_diaobjects,self.paginate_by)
            page=self.request.GET.get('page')
            if page:
        ...
        if self.request.GET.get('colororigin'):
            all_diaobjects,colororigin=self.get_by_colororigin(self.request.GET.get('colororigin'),all_diaobjects)
            context['total_count']=self.get_object_count(all_diaobjects)
            paginator=Paginator(all_diaobjects,self.paginate_by)
            page=self.request.GET.get('page')
            if page:
        ...
        all_diaobjects,colororigins=self.get_colororigin_filtered(all_diaobjects)
        context['total_count'] = self.get_object_count(all_diaobjects)
        paginator = Paginator(all_diaobjects, self.paginate_by)
        page = self.request.GET.get('page')
        if page:
    ...
    def get_colororigin_filtered(self,all_diaobjects):
        ...
        if(len(colororigins)>0):
            all_diaobjects=all_diaobjects.filter(color_origin1__c_o__in=colororigins)
