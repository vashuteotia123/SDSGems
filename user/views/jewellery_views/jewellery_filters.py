...
class jewelleryFilter(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.GET.get('centerstone'):
            ...
            context['fluorescencecolors'] = SafeString(str(centerstones))
            context['all_jewobjects'] = all_jewobjects
            return context
        if self.request.GET.get('colorofcenterstone'):
            ...
            context['fluorescencecolors'] = SafeString(str(colorofcenterstones))
            context['all_jewobjects'] = all_jewobjects
            return context
        ...
        all_jewobjects,centerstones=self.get_centerstone_filtered(all_jewobjects)
        all_jewobjects,colorofcenterstones=self.get_colorofcenterstone_filtered(all_jewobjects)
    ...
    def get_metal_filtered(self,all_jewobjects):
        ...
        if(len(metals)>0):
            all_jewelobjects=all_jewobjects.filter(metal__metal__in=metals)
        return all_jewelobjects,metals
    def get_centerstone_filtered(self,all_jewobjects):
        centerstones=self.request.GET.getlist('centerstones[]')
        if(len(centerstones)>0):
            all_jewobjects=all_jewobjects.filter(center_stone__stone__in=centerstones)
        return all_jewobjects,centerstones
    def get_type_filtered(self,all_jewobjects):
        types=self.request.GET.getlist('types[]')
        if(len(types)>0):
            all_jewobjects=all_jewobjects.filter(jewellery_type__jewel__in=types)
    ...
    def get_colorofcenterstone_filtered(self,all_jewobjects):
        ...
        if(len(colorofcenterstones)>0):
            all_jewobjects=all_jewobjects.filter(color_of_center_stone__color__in=colorofcenterstones)
    ...
    def get_by_colorofcenterstone(self,colorofcenterstone,all_jewobjects):
        all_jewobjects=all_jewobjects.filter(color_of_center_stone__color=colorofcenterstone)
    ...
    def get_high_to_low(self, all_jewobjects):
        ...
        if len(HighToLow) > 0:
            return all_jewobjects.order_by('-tag_price_d'), 1
